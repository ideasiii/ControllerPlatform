...
public:
	...
	int openRdmDB(const char *dbPath);
	int controllerSqlExec(const char *szSql);
	int rdmSqlExec(const char *szSql);
	int rdmSqlExec(const char *szSql, std::list<std::string> &listValue, int nColumeIndex);
