...
typedef struct
{
		...
		std::string strMAC;
		std::string strMongoDBControllerIP;
		std::string strMongoDBControllerPort;
		std::string strMongoDBControllerMsgID;
...
class Controller: public CObject
{
	...
	private:
		...
		int sendCommandtoSocketServer(int socketFD,int nCommand, int nStatus, int nSequence, bool isResp);
		...
		int cmpBindRequest(const int nSocket,CSocketClient * client);
	...
	private:
		...
		CSocketClient *mongoDBClient;
		std::vector<CSocketClient *> clientCollection;
