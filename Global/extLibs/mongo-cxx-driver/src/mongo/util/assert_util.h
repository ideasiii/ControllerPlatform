...
enum CommonErrorCodes {
	OkCode = 0, DatabaseDifferCaseCode = 13297,    // uassert( 13297 )
...
struct MONGO_CLIENT_API ExceptionInfo {
	...
	void append(BSONObjBuilder& b, const char* m = "$err", const char* c =
			"code") const;
...
public:
	...
	Status toStatus(const std::string& context) const {
		return Status(convertExceptionCode(getCode()),
				context + causedBy(*this));
	}
...
MONGO_CLIENT_API MONGO_COMPILER_NORETURN void MONGO_CLIENT_FUNC
invariantOKFailed(const char* expr, const Status& status, const char* file,
		unsigned line);
MONGO_CLIENT_API void MONGO_CLIENT_FUNC
wasserted(const char* expr, const char* file, unsigned line);
MONGO_CLIENT_API MONGO_COMPILER_NORETURN void MONGO_CLIENT_FUNC fassertFailed(
		int msgid);
MONGO_CLIENT_API MONGO_COMPILER_NORETURN void MONGO_CLIENT_FUNC
...
MONGO_CLIENT_API std::string MONGO_CLIENT_FUNC causedBy(const DBException& e);
MONGO_CLIENT_API std::string MONGO_CLIENT_FUNC causedBy(
...
MONGO_CLIENT_API inline void MONGO_CLIENT_FUNC fassert(int msgid,
		const Status& status) {
...
MONGO_CLIENT_API inline void MONGO_CLIENT_FUNC uassertStatusOK(
		const Status& status) {
	if (MONGO_unlikely(!status.isOK())) {
